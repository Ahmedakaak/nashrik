...
import {
    Megaphone, Plus, Edit3, Trash2, ChevronDown, ChevronUp,
    AlertCircle, X, Clock
} from 'lucide-react'

const initialForm = { title: '', title_ar: '', content: '', content_ar: '', priority: 'normal' }
...
export default function AnnouncementsPage() {
    ...
    const openEdit = (ann) => {
        setEditingAnn(ann)
        setForm({ title: ann.title, title_ar: ann.title_ar, content: ann.content, content_ar: ann.content_ar, priority: ann.priority })
        setShowModal(true)
    }

    const handleSave = () => {
        if (editingAnn) {
            setAnnouncements(prev => prev.map(a => a.id === editingAnn.id ? { ...a, ...form } : a))
        } else {
            const newAnn = {
                ...form, id: `a-${Date.now()}`, club_id: '1',
                created_at: new Date().toISOString(), author: 'You',
            }
    ...
    const priorityColors = {
        ...
        low: 'bg-surface-darker text-text-muted border-surface-border',
    }

    const formatDate = (dateStr) => {
        return new Date(dateStr).toLocaleDateString(isRTL ? 'ar-OM' : 'en-US', {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-text-primary">{t('clubAdmin.announcements.title')}</h1>
                    <p className="text-text-secondary mt-1">{announcements.length} announcements</p>
                </div>
                ...
                <AnimatePresence>
                    {announcements.map(ann => (
                        <motion.div
                            ...
                            className={`bg-surface-card border rounded-2xl overflow-hidden transition-colors ${expanded === ann.id ? 'border-brand-400/30' : 'border-surface-border hover:border-brand-400/20'}`}
                        >
                            <div
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className={`text-xs px-2 py-0.5 rounded-lg font-medium shrink-0 ${priorityColors[ann.priority]}`}>
                                                {ann.priority}
                                            </span>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                        <button onClick={e => { e.stopPropagation(); openEdit(ann) }} className="p-2 rounded-lg hover:bg-white/5 text-text-muted hover:text-brand-400 transition-colors">
                                            <Edit3 size={15} />
                                        </button>
                                        <button onClick={e => { e.stopPropagation(); handleDelete(ann.id) }} className="p-2 rounded-lg hover:bg-white/5 text-text-muted hover:text-status-error transition-colors">
                                            <Trash2 size={15} />
                                        </button>
                ...
                {showModal && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-surface-card border border-surface-border rounded-2xl w-full max-w-lg"
                        ...
                        >
                            ...
                            <div className="p-5 space-y-4">
                                ...
                                <div>
                                    <label className="text-sm text-text-secondary mb-1 block">Content (EN)</label>
                                    <textarea rows={3} value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} className="w-full px-3 py-2.5 bg-surface-darker border border-surface-border rounded-xl text-text-primary text-sm resize-none" />
                                </div>
                                <div>
                                    <label className="text-sm text-text-secondary mb-1 block">المحتوى (AR)</label>
                                    <textarea dir="rtl" rows={3} value={form.content_ar} onChange={e => setForm({ ...form, content_ar: e.target.value })} className="w-full px-3 py-2.5 bg-surface-darker border border-surface-border rounded-xl text-text-primary text-sm resize-none" />
                                </div>
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        {['normal', 'high', 'low'].map(p => (
                                            <button
                                                key={p} onClick={() => setForm({ ...form, priority: p })}
                                                className={`px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${form.priority === p ? priorityColors[p] : 'border-surface-border text-text-muted hover:bg-white/5'}`}
                                            >
